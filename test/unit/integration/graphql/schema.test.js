...
describe('schema', () => {
  ...
  it('should only contain fields that have the directive', async () => {
    ...
    expect(
      findBreakingChanges(
        schema,
        buildSchema(await readFile(join(dirname(fileURLToPath(import.meta.url)), 'partial-schema.gql'), 'utf-8'))
      )
    ).toHaveLength(0)
  })
  ...
  it('should contain all fields if process.env.ALL_SCHEMA is set', async () => {
    ...
    expect(
      findBreakingChanges(
        schema,
        buildSchema(await readFile(join(dirname(fileURLToPath(import.meta.url)), 'full-schema.gql'), 'utf-8'))
      )
    ).toHaveLength(0)
  })
})
